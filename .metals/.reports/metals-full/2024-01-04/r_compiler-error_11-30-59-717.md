...
import spark.implicits._
  ...
  // DF  
  val correlationMatrix = Correlation.corr(assembledData, "features").collect(0)(@@)
  // Vector
  val similarityMatrix: Vector = Vectors.dense(correlationMatrix.toArray)
  // similarityMatrix.toArray.foreach(println)

  // Example usage
  val topSimilarUsers = findTopSimilarUsers(1, similarityMatrix, 5)
  println(s"Top Similar Users for user_id 1:")
  topSimilarUsers.foreach(println)

  def findTopSimilarUsers(userID: Int, similarityMatrix: Vector, numSimilarUsers: Int): Array[(Int, Double)] = {
    val userIndex = indexedUserItemMatrix.select("user_id", "user_id_indexed")
      .distinct().filter($"user_id" === userID).first().getInt(1)
    val userSimilarities = similarityMatrix.toArray.zipWithIndex.filter(_._2 != userIndex)
